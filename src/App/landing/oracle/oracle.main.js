agmNgModuleWrapper("agm.landing")
    .defineControllerAsPage("agm.landing.OracleController",
    "/App/landing/oracle/oracle.main.html",
    [],
        ...
        tool.onRendered(function () {
            ...
            function blackhole(element) {
                var h = $(element).height(),
                    ...
                    cw = w,
                    ch = h,
                    maxorbit = 400, // distance from center
                    centery = ch / 2,
                    centerx = cw / 2;
                ...
                var star = function () {
                    ...
                    var rands = [];
                    rands.push(Math.random() * (maxorbit / 300000) + 12);
                    ...
                    this.speed = (Math.floor(Math.random() * 2.5) + 1.5) * Math.PI / 120; // The rate at which this star will orbit
                    ...
                    this.collapseBonus = this.orbital - (maxorbit * 1.0); // This "bonus" is used to randomly place some stars outside of the blackhole on hover
                    if (this.collapseBonus < 1) { // if the collapse "bonus" is negative
                        this.collapseBonus = 0; // set it to 0, this way no stars will go inside the blackhole
                    }

                    stars.push(this);
                    this.color = 'rgba(0,100,255,' + (1.2 - ((this.orbital) / 255)) + ')'; // Color the star white, but make it more transparent the further out it is generated

                    this.hoverPos = centery + (maxorbit / 1.2) + this.collapseBonus;  // Where the star will go on hover of the blackhole
                    this.expansePos = centery + (this.id % 100) * -10 + (Math.floor(Math.random() * 200) + 1); // Where the star will go when expansion takes place
                ...
                star.prototype.draw = function () {
                    ...
                    if (!expanse) {
                        ...
                        this.rotation = this.startRotation + (currentTime * (this.speed / 5));
                ...
                $('.centerHover').on('click', function () {
                    ...
                    $(this).addClass('openx');
                    $('.fullpage').addClass('openx');
                    setTimeout(function () {
                        $('.header .welcome').removeClass('gonex');
                    }, 500);
                });
                ...
                function loop() {
                    ...
                    context.fillStyle = 'rgba(0,0,10,0.2)'; // somewhat clear the context, this way there will be trails behind the stars 
