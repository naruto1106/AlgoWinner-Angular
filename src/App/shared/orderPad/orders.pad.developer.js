agmNgModuleWrapper('agms.orders', [])
    .defineControllerAsPopup('s.orders.PadDeveloperPopupController',
    {
        templateUrl: '/App/shared/orderPad/orders.pad.developer.html',
        windowClass: 'smaller-modal'
    },
    [
        'sProductService', 'sStrategyCommerceService', "sOrdersBracketService", 
        'orderService', 'orderPadInitService', 'myStrategies', 'reloadStrategiesPromiseFunc', 'myTradingAccounts',
    ...
    function (vm, dep, tool) {
        ...
        var $filter = dep.$filter,
            ...
            reloadStrategiesPromiseFunc = dep.reloadStrategiesPromiseFunc,
        ...
        function openNewStrategy() {
            return sStrategyCommerceService.openNewStrategyWithoutReload().result.then(function () {
                if (reloadStrategiesPromiseFunc) {
                    reloadStrategiesPromiseFunc()
                        .then(function (obj) {
                            vm.myTradingAccounts = obj.accounts;
                            vm.allStrategies = obj.strategies;
                            return sOrdersPadHelperService.refreshStrategyFlags(vm.allStrategies, vm.order); // wait flagging all strategies
                        })
                        .then(function () {
                            if (!vm.selectedStrategy) {
                                vm.selectedStrategy = vm.allStrategies[0];
                            }
                            return onSelectedStrategy();
                        });
                };
            });
        }
        ...
        function disableConfirm() {
            ...
            return (vm.validationMessage !== null && vm.validationMessage !== "") || isInvalidStrategy || needCapitalTopup() || vm.isLoadingPrice;
        }

        function needCapitalTopup() {
            if (!vm.selectedStrategy) {
                return false;
            }
            return vm.selectedStrategy.CapitalInfo.TradingCapital <= 0 && vm.isEntering();
        }
        ...
        tool.initialize(function () {
            tool.setVmProperties({
                ...
                needCapitalTopup: needCapitalTopup,
