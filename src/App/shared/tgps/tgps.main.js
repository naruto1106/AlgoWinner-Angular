            ...
            'mode'], function (vm, dep, tool) {
                ...
                function calculateFundamentalFilterNumber(fundamentalParamsList) {
                    ...
                    fundamentalParamsList.forEach(function (f) {
                        if (vm.filter.tradeVenueLoc && f.TradeVenue === vm.filter.tradeVenueLoc.label) {
                            count = count +
                ...
                function onFrontEndFilterChanged() {
                    if (vm.mode === 'Position') {
                        vm.filteredStocksForPosition = allStocksForPosition.filter(function (s) {
                            ...
                            if (vm.filter.tidChange === 'All') {
                                return true;
                            }
                            if (vm.filter.tidChange === -1) {
                                return s.TIDChange === vm.filter.tidChange;
                            }
                            if (vm.filter.tidChange === 0) {
                                return s.TIDChange === vm.filter.tidChange;
                            }
                            if (vm.filter.tidChange === 1) {
                                return s.TIDChange === vm.filter.tidChange;
                ...
                function loadPositionStockList(fundamentalParamsList) {
                    ...
                    if (isWorldIndices()) {
                        ...
                        if (coreUserStateService.hasTradersGPSPlus() && vm.mode === 'Plus') {
                            ...
                            if (vm.filter.watchlist) {
                            ...
                            } else if (vm.filter.predefinedWatchlist) {
                                var request3 = {
                                    Grouping: vm.filter.predefinedWatchlist,
                                    BarSize: vm.filter.barSize.value,
                                    GetByLastTradingDate: vm.filter.dateSelectionMode === 0,
                                    TriggerDate: date,
                                    UseFundamentalFilter: sTgpsScreenerService.fundamentalFilterNumber > 0,
                                ...
                                p = sTgpsService.getTradersGPSPositionGroupingScreener(request3);
                            }
                            else {
                                var request2 = {
                ...
                function loadSwingStockList(fundamentalParamsList) {
                    ...
                    if (isWorldIndices()) {
                        ...
                        if (vm.filter.watchlist) {
                            ...
                            p = sTgpsService.getTradersGPSSwingGroupingScreener(request3);
                        }
                        else {
                            var request2 = {
                                TradeVenue: vm.filter.tradeVenueLoc.label,
                ...
                function updateSelection(param) {
                    if (param.type === 'TradeVenue') {
                        ...
                        vm.filter.predefinedWatchlist = null;
                    } else if (param.type === 'Watchlist') {
                        vm.filter.watchlist = param.value;
                        vm.filter.tradeVenueLoc = null;
                        vm.filter.predefinedWatchlist = null;
                    } else if (param.type === 'PredefinedWatchlist') {
                        vm.filter.predefinedWatchlist = param.value;
                        vm.filter.tradeVenueLoc = null;
                        vm.filter.watchlist = null;
                    }
                ...
                function hasFundamentalFilterSelections() {
                    return (vm.filter.tradeVenueLoc && vm.filter.tradeVenueLoc.label !== 'Global Indices') || vm.filter.watchlist || vm.filter.predefinedWatchlist;
                }

                function isWorldIndices() {
                    return vm.filter.tradeVenueLoc && !vm.filter.predefinedWatchlist && vm.filter.tradeVenueLoc.label === 'Global Indices';
                }

                function isUSMarket() {
                    return vm.filter.tradeVenueLoc && !vm.filter.predefinedWatchlist && vm.filter.tradeVenueLoc.label !== 'SG' && vm.filter.tradeVenueLoc.label !== 'HK' && vm.filter.tradeVenueLoc.label !== 'MY' && vm.filter.tradeVenueLoc.label !== 'CHN';
                }

                function isNotMYMarket() {
                    if (vm.filter.tradeVenueLoc && !vm.filter.predefinedWatchlist)
                        return (vm.filter.tradeVenueLoc.label === 'SG' || vm.filter.tradeVenueLoc.label === 'HK' || vm.filter.tradeVenueLoc.label === 'US' || vm.filter.tradeVenueLoc.label === 'CHN');
                    return false;
                }
                ...
                var productListOptionForPosition = {
                    ...
                    columns: [
                        ...
                        },
                        {
                            ...
                            sortingFunc: createSortingFunc('TID'),
                            sortingDirection: -1
                        },
                        {
                ...
                function showLastAvailableDay() {
                    return vm.filter.dateSelectionMode === 0 &&
                        (vm.filter.watchlist != null || vm.filter.predefinedWatchlist != null || vm.filter.tradeVenueLoc.label !== 'Global Indices') &&
