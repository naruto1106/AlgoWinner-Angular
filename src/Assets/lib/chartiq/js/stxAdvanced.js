...
*/		
		    ...
		    function getMarketOffset(localQuoteDate) {
		        ...
		        if (!isForex) {
		            if (stx.chart.market) {
		                ...
		                var foreignExchange = STX.Market.Symbology.isForeignSymbol(stx.chart.symbol) && stx.chart.symbol.split(".").pop();
		    ...
		    for(var i=1;i<quotes.length;i++){
		    	...
		    	if(period=="year" && quotes[i].DT.getYear()!=quotes[i-1].DT.getYear()){
		    		...
		    		if(marketOffset===null){
		    			...
		    			marketOffset=getMarketOffset(stx,quotes[i].DT);
			    		if(isForex){
		...
		STX.Drawing.fibtimezone.prototype.render=function(context){
			...
			var txt=0;
			fibColor=this.parameters.timezone.color;
			if(fibColor=="auto" || STX.isTransparent(fibColor)) fibColor=this.color;
			if(fibColor=="auto" || STX.isTransparent(fibColor)) fibColor=this.stx.defaultColor;
		...
		*/
		...
		STX.Marker.AboveCandle.placementFunction = function (params) {
		    ...
		    for (var i = 0; i < params.arr.length; i++) {
		        ...
		        if (marker.params.xPositioner == "bar") {
		            ...
		            if (marker.tick < params.firstTick && marker.rightEdge < 0) 
                    {
                        node.style.left = "-1000px";
                        continue; // off screen, no need to reposition the marker
                    }
        ...
        /* Patching by Algomerchant due to following reason.
            Quote has two properties for dates, DT and displayDate
            The bar shown in the chart could have different displayDate than DT.
            marker.tick which is pre-set by setMarkerTick uses DT. 
            But displayDate is used as the label on the x-axis of the chart.
            So, marker could appear few bars before a bar.
            Eg :
                In a 5minute size bar chart, 
                    bar1's DT=4:33 and displayDate=4:30
                    bar2's DT=4:38 and displayDate=4:35
                a marker which has time 4:36 would end up on bar1 although it should appear on bar2
        
        */
		function getMatchingPositionAfterAlgomerchantPatching(marker, chartDataSet) {
		    if (marker.params.xPositioner !== "date") return null;
	        if (!marker.params.x) return null; // x has the date of the marker when xPositioner is "date"
		    var quote = chartDataSet[marker.tick];
		    if (!quote.displayDate) return null; // no displayDate means master data. no patching needed for those.
		    for (var i = marker.tick + 1; i < chartDataSet.length; i++) {
		        var nextQuote = chartDataSet[i];
		        if (nextQuote.displayDate > marker.params.x // Making sure marker is displayed on rounded down quote
                    || Math.abs(marker.params.x - quote.displayDate) <= Math.abs(marker.params.x - nextQuote.displayDate)) {
		            marker.tick = i - 1;
		            break;
		        }
		        quote = nextQuote;
		    ...
		    for (var i = 0; i < params.arr.length; i++) {
		        ...
		        if (marker.params.xPositioner != "none") {
		            if (marker.params.xPositioner == "bar" && marker.params.x) {
		                ...
		                if (marker.tick < chart.dataSet.length) {
		                    ...
		                    var algomerchantPatchedQuote = getMatchingPositionAfterAlgomerchantPatching(marker, chart.dataSet);
		                    quote = algomerchantPatchedQuote || quote;
		                }
		                if (marker.tick < params.firstTick && marker.rightEdge < 0) {
                            node.style.left = "-1000px";
                            continue; // off screen, no need to reposition the marker
		        ...
		        if (marker.params.yPositioner != "none") {
		            ...
		            if (marker.params.yPositioner == "value" && marker.params.y) {
		                ...
		                //above_candle
		                if (showsHighs) {
		                    bottom = Math.round(height - stx.pixelFromPriceTransform(quote.High, panel, yAxis) + bottomAdjust) + 10 + "px";
		                } else {
		                    bottom = Math.round(height - stx.pixelFromPriceTransform(quote.Close, panel, yAxis) + bottomAdjust) + 10 + "px";
		                }
		            }
		            if (node.style.bottom != bottom) node.style.bottom = bottom;
		        }
