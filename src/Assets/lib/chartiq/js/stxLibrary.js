...
(function(){
	...
	function _stxLibrary_js(_stxThirdParty,_exports) {
		...
		STX.Studies.createVolumeChart=function(stx, sd, quotes){
            ...
            // Do not draw volume graph for index product
            if (sd.inputs.isIndexProduct) {
                stx.watermark(sd.panel, "center", "top", stx.translateIf("No data available"));
                return;
            } 
			stx.drawHistogram(params,seriesParam);
        };
		...
		STX.Drawing.segment.prototype.intersected=function(tick, value, box){
			...
			if(isIntersected){
				...
				return {
					...
					value: this.stx.chart.isComparison? this.stx.adjustIfNecessary(
                            this.stx.chart.panel, 
                            this.stx.crosshairTick, 
                            this.stx.valueFromPixelUntransform(this.stx.backOutY(STXChart.crosshairY), this.stx.chart.panel)): value // save original value
				};
			}else{
				return null;
			}
